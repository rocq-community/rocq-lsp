...
module DefAttributes = struct
  ...
  (* [locality] is used for [vernac_definition_hook], the raw Local/Global
     attribute is also used to generate [scope]. [locality] can't be computed
     back from [scope] because [Let Coercion] outside section generates
     [locality = None] but [scope = Global ImportNeedQualified] (which is
     otherwise associated with [locality = Some true]).

     Since [Let] (ie discharge = DoDischarge) does not allow explicit locality
     we could alternatively decide to change the default locality of the
     coercion from out-of-section [Let Coercion]. *)
  ...
  let warn_declaration_outside_section =
    CWarnings.create ~name:"my-declaration-outside-section"
      ~category:CWarnings.CoreCategories.vernacular ~default:CWarnings.AsError
      Pp.(
        fun (unexpected_thing, replacement) ->
          strbrk "Use of " ++ str unexpected_thing
          ++ strbrk " outside sections behaves as "
          ++ str replacement ++ str ".")

  let scope_of_locality locality_flag discharge deprecated_thing replacement :
      definition_scope =
    ...
    | None, DoDischarge when not (Lib.sections_are_opened ()) ->
      (* If a Let/Variable is defined outside a section, then we consider it as
         a local definition *)
      warn_declaration_outside_section (deprecated_thing, replacement);
      Global ImportNeedQualified
    | None, DoDischarge -> Discharge
    | Some true, DoDischarge ->
      CErrors.user_err Pp.(str "Local not allowed in this case")
    | Some false, DoDischarge ->
      CErrors.user_err Pp.(str "Global not allowed in this case")
  ...
  let parse ?(coercion = false) ?(discharge = (NoDischarge, "", "")) f =
    ...
    let clearbody =
      ...
      | DoDischarge -> clearbody
      | NoDischarge -> return None
    in
    let ( ( ( ( ( (((locality, user_warns), polymorphic), program)
        ...
        , clearbody ) =
      parse
        (locality ++ user_warns ++ polymorphic ++ program ++ canonical_instance
       ++ typing_flags ++ using ++ reversible ++ clearbody)
        f
    in
    let using = Option.map Proof_using.using_from_string using in
    let reversible = Option.default false reversible in
    let () =
      if Option.has_some clearbody && not (Lib.sections_are_opened ()) then
        CErrors.user_err
          Pp.(str "Cannot use attribute clearbody outside sections.")
    in
    let scope =
      scope_of_locality locality discharge deprecated_thing replacement
    in
    { scope
    ; locality
    ; polymorphic
...
(** constr_expr -> glob_constr translation:
    ...
    - it prepares pattern matching problems (a pattern becomes a tree where
      nodes are constructor/variable pairs and leafs are variables)

    All that at once, fasten your seatbelt! *)

(* To interpret implicits and arg scopes of variables in inductive types and
   recursive definitions and of projection names in records *)
...
let explain_variable_capture id id' =
  Id.print id
  ++ str " is dependent in the type of "
  ++ Id.print id'
  ++ strbrk ": cannot interpret both of them with the same type"

let explain_illegal_metavariable = str "Metavariables allowed only in patterns"
let explain_not_a_constructor qid = str "Unknown constructor: " ++ pr_qualid qid

let explain_unbound_fix_name is_cofix id =
  str "The name" ++ spc () ++ Id.print id ++ spc ()
  ++ str "is not bound in the corresponding"
  ++ spc ()
  ++ str (if is_cofix then "co" else "")
  ++ str "fixpoint definition"
...
let explain_bad_patterns_number n1 n2 =
  str "Expecting " ++ int n1
  ++ str (String.plural n1 " pattern")
...
let explain_projections_of_diff_records record1 record2 =
  ...
  str "This record contains fields of both "
  ++ pr_qualid inductive1_id ++ str " and " ++ pr_qualid inductive2_id
...
let _ =
  CErrors.register_handler (function
    | InternalizationError e -> Some (explain_internalization_error e)
    | _ -> None)

let error_bad_inductive_type ?loc ?info () =
  user_err ?loc ?info
    (str "This should be an inductive type applied to patterns.")

let error_parameter_not_implicit ?loc =
  user_err ?loc
    (str "The parameters do not bind in patterns;"
    ++ spc ()
    ++ str "they must be replaced by '_'.")

let error_ldots_var ?loc =
  user_err ?loc
    (str "Special token " ++ Id.print ldots_var
...
let compute_internalization_data env sigma ?silent id ty typ impl =
  let impl =
    compute_implicits_with_manual env sigma ?silent typ (is_implicit_args ())
      impl
  in
...
let contract_curly_brackets ntn (l, ll, bl, bll) =
  ...
  | InConstrEntry, ntn ->
    ...
    let rec contract_squash n = function
      | [] -> []
      | { CAst.v =
            CNotation (None, (InConstrEntry, "{ _ }"), ([ a ], [], [], []))
        }
        :: l ->
...
let contract_curly_brackets_pat ntn (l, ll, bl) =
  ...
  | InConstrEntry, ntn ->
    ...
    let rec contract_squash n = function
      | [] -> []
      | { CAst.v =
            CPatNotation (None, (InConstrEntry, "{ _ }"), ([ a ], [], []), [])
        }
        :: l ->
        ntn' := expand_notation_string !ntn' n;
        contract_squash n (a :: l)
...
type local_univs =
  { bound : UnivNames.universe_binders
  ; unb_univs : bool
  }
...
type intern_env =
  ...
  ; strict_check : bool option
        (* None = not passed via ltac yet: works as "true" unless when
           interpreting ltac:() in which case we assume the default Ltac value,
           that is "false" *)
...
type pattern_intern_env =
  ...
  ; (* ids = Some means accept local variables; this is useful for terms as
       patterns parsed as patterns in notations *)
...
let make_current_scope tmp scopes =
  match (tmp, scopes) with
  | [], scopes -> scopes
...
let pr_scope_stack begin_of_sentence l =
  let bstr x =
    if begin_of_sentence then str (CString.capitalize_ascii x) else str x
  in
  match l with
  | [] -> bstr "the empty scope stack"
  | [ a ] -> bstr "scope " ++ str a
  | l ->
    bstr "scope stack " ++ str "[" ++ prlist_with_sep pr_comma str l ++ str "]"

let warn_inconsistent_scope =
  CWarnings.create ~name:"my-inconsistent-scopes"
    ~category:CWarnings.CoreCategories.syntax (fun (id, scopes1, scopes2) ->
...
let set_notation_var_scope ?loc id ((tmp_scope, subscopes) as scopes) ntnbinders
    ntnvars =
  try
    let ({ Genintern.ntnvar_typ = typ } as status) = Id.Map.find id ntnvars in
    match typ with
    | Notation_term.NtnInternTypeOnlyBinder ->
      error_expect_binder_notation_type ?loc id
    | Notation_term.NtnInternTypeAny principal ->
      let () =
        match status.ntnvar_scopes with
        | None -> status.ntnvar_scopes <- Some scopes
        | Some (tmp_scope', subscopes') ->
          let s' = make_current_scope tmp_scope' subscopes' in
          let s = make_current_scope tmp_scope subscopes in
          if Option.is_empty principal && not (List.equal String.equal s' s)
          then warn_inconsistent_scope ?loc (id, s', s)
      in
      let () =
        match status.ntnvar_binding_ids with
        | None -> status.ntnvar_binding_ids <- Some ntnbinders
        | Some ntnbinders' ->
          status.ntnvar_binding_ids <-
            Some (Id.Set.inter ntnbinders ntnbinders')
      in
      let () =
        match status.ntnvar_used with
...
let set_type_scope env =
  { env with tmp_scope = Notation.current_type_scope_names () }
...
let mkGProd ?loc (na, bk, t) body =
  DAst.make ?loc @@ GProd (na, None, bk, t, body)

let mkGLambda ?loc (na, bk, t) body =
  DAst.make ?loc @@ GLambda (na, None, bk, t, body)

(**********************************************************************)
(* Utilities for binders                                              *)

let warn_shadowed_implicit_name =
  CWarnings.create ~name:"my-shadowed-implicit-name"
    ~category:CWarnings.CoreCategories.syntax
    Pp.(
      fun na ->
        str "Making shadowed name of implicit argument accessible by position.")
...
let exists_name na l =
  ...
  | Name id ->
    List.exists
      (function
        | Some { impl_pos = Name id', _, _ } -> Id.equal id id'
        | _ -> false)
      l
  | _ -> false
  ...
  let impl_status max =
    let na =
      if exists_name na acc then (
        warn_shadowed_implicit_name ?loc na;
        Anonymous)
      else na
    in
    Some
...
let impls_binder_list =
  let rec aux acc n = function
    | (na, _, bk, None, _) :: binders ->
      aux (build_impls n bk na acc) (n + 1) binders
...
let impls_term_list n ?(args = []) =
  let rec aux acc n c =
    ...
    | GRec (fix_kind, nas, args, tys, bds) ->
      ...
      let n, acc' =
        List.fold_left
          (fun (n, acc) (na, _, bk, _, _) -> (n + 1, build_impls n bk na acc))
          (n, acc) args.(nb)
      in
      aux acc' n bds.(nb)
    | _ -> List.rev acc
...
(* [test_kind_strict] rules out pattern which refers to global other than
   constructors or variables; It is used in instances of notations *)

let test_kind_pattern_in_notation ?loc = function
  | GlobRef.ConstructRef _ ->
    ()
    (* We do not accept non constructors to be used as variables in patterns *)
  | GlobRef.ConstRef _ ->
...
let test_kind_ident_in_notation ?loc = function
  | GlobRef.ConstructRef _ ->
    user_err ?loc
      (str "Found a constructor while a variable name was expected.")
  | GlobRef.ConstRef _ ->
    user_err ?loc (str "Found a constant while a variable name was expected.")
  | GlobRef.IndRef _ ->
    user_err ?loc
      (str "Found an inductive type while a variable name was expected.")
  | GlobRef.VarRef _ ->
    (* we accept a section variable name to be used as pattern variable *)
    raise Not_found

(* [test_kind_tolerant] allow global reference names to be used as pattern
   variables *)
...
let locate_if_hole ?loc na c =
  ...
  | GHole _ -> (
    try
      match na with
      | Name id ->
        glob_constr_of_notation_constr ?loc (Reserve.find_reserved_type id)
...
let pure_push_name_env (id, implargs, is_ntn_id) env =
  ...
  ; ntn_binding_ids =
      (if is_ntn_id then Id.Set.add id env.ntn_binding_ids
       else env.ntn_binding_ids)
  }

let push_name_env ~dump (ntnvars : Genintern.ntnvar_status Id.Map.t) implargs
    env =
  let open CAst in
  function
  | { loc; v = Anonymous } -> env
  | { loc; v = Name id } ->
    if Id.Map.is_empty ntnvars && Id.equal id ldots_var then
      error_ldots_var ?loc;
    set_var_is_binder ?loc id ntnvars;
    let uid = var_uid id in
    if dump then Dumpglob.dump_binding ?loc uid;
    pure_push_name_env
      (id, make_var_data Variable implargs [] uid, Id.Map.mem id ntnvars)
      env
...
let remember_binders_impargs env bl =
  List.map_filter
    (fun (na, _, _, _, _) ->
      ...
      | Anonymous -> None
      | Name id ->
        Some (id, Id.Map.find id env.impls, Id.Set.mem id env.ntn_binding_ids))
    bl
...
let warn_ignoring_unexpected_implicit_binder_declaration =
  CWarnings.create ~name:"my-unexpected-implicit-declaration"
    ~category:CWarnings.CoreCategories.syntax
    Pp.(
      fun () ->
        str "Ignoring implicit binder declaration in unexpected position.")

let check_implicit_meaningful ?loc k env =
  if k <> Explicit && env.binder_block_names = None then (
    warn_ignoring_unexpected_implicit_binder_declaration ?loc ();
    Explicit)
  else k

let intern_generalized_binder ~dump intern_type ntnvars env { loc; v = na } b' t
    ty =
  let ids =
    (match na with
    | Anonymous -> fun x -> x
    | Name na -> Id.Set.add na)
      env.ids
  in
  let ty, ids' =
    if t then (ty, ids) else Implicit_quantifiers.implicit_application ids ty
  in
  let ty' = intern_type { env with ids; strict_check = Some false } ty in
  let fvs =
    Implicit_quantifiers.generalizable_vars_of_glob_constr ~bound:ids
      ~allowed:ids' ty'
  in
  let env' =
    List.fold_left
      (fun env { loc; v = x } ->
        push_name_env ~dump ntnvars [] (*?*) env (make ?loc @@ Name x))
      env fvs
  in
    ...
    List.map
      CAst.(
        map (fun id ->
            ( Name id
            , MaxImplicit
            , DAst.make ?loc @@ GHole (GBinderType (Name id)) )))
      fvs
  in
  let na =
    match na with
    | Anonymous ->
      ...
      in
      let ids' =
        List.fold_left (fun ids' lid -> Id.Set.add lid.CAst.v ids') ids' fvs
      in
      let id = Implicit_quantifiers.make_fresh ids' (Global.env ()) id in
      Name id
    | _ -> na
  in
...
let intern_assumption ~dump intern (ntnvars : Genintern.ntnvar_status Id.Map.t)
    env nal bk ty =
  ...
  | Generalized (b', t) ->
    let env, b =
      intern_generalized_binder ~dump intern_type ntnvars env (List.hd nal) b' t
        ty
    in
    (env, b)

let glob_local_binder_of_extended =
  DAst.with_loc_val (fun ?loc -> function
    ...
    | GLocalPattern (_, _, _, _) ->
      Loc.raise ?loc
        (Gramlib.Grammar.ParseError "Pattern with quote not allowed here."))

let intern_cases_pattern_fwd =
  ref (fun _ -> failwith "intern_cases_pattern_fwd")

let intern_letin_binder ~dump intern ntnvars env
    (({ loc; v = na } as locna), def, ty) =
  ...
  (push_name_env ~dump ntnvars impls env locna, (na, term, ty))

let intern_cases_pattern_as_binder ~dump intern test_kind ntnvars env bk
    (CAst.{ v = p; loc } as pv) =
  let p, t, tmp_scope =
    match p with
    | CPatCast (p, t) ->
      ( p
      , Some t
      , (* Redone later, not nice: *)
        Notation.compute_glob_type_scope (intern (set_type_scope env) t) )
    | _ -> (pv, None, [])
  in
  let il, disjpat =
    let il, subst_disjpat =
      !intern_cases_pattern_fwd test_kind ntnvars
        (env_for_pattern { env with tmp_scope })
        p
    in
    ...
    if
      not
        (List.for_all
           (fun subst -> Id.Map.equal Id.equal subst Id.Map.empty)
           substl)
    then user_err ?loc (str "Unsupported nested \"as\" clause.");
    (il, disjpat)
  in
  ...
  let env =
    List.fold_right
      (fun { loc; v = id } env ->
        push_name_env ~dump ntnvars [] env (make ?loc @@ Name id))
      il env
  in
  ...
  let t =
    match t with
    | Some t -> t
    | None -> CAst.make ?loc @@ CHole (Some (GBinderType na.v))
  in
  let _, bl' =
    intern_assumption ~dump intern ntnvars env [ na ] (Default bk) t
  in
  let { v = _, bk, t } = List.hd bl' in
  let il = List.map (fun id -> id.v) il in
  (env, ((disjpat, il), id), bk, t)

let intern_local_binder_aux ~dump intern
    (ntnvars : Genintern.ntnvar_status Id.Map.t) (env, bl) = function
  | CLocalAssum (nal, _, bk, ty) ->
    let env, bl' = intern_assumption ~dump intern ntnvars env nal bk ty in
    let bl' =
      List.map
        (fun { loc; v = na, c, t } ->
          DAst.make ?loc @@ GLocalAssum (na, None, c, t))
        bl'
    in
    (env, bl' @ bl)
  | CLocalDef (({ loc; v = na } as locna), _, def, ty) ->
    let env, (na, def, ty) =
      intern_letin_binder ~dump intern ntnvars env (locna, def, ty)
    in
    (env, (DAst.make ?loc @@ GLocalDef (na, None, def, ty)) :: bl)
  | CLocalPattern p ->
    let env, ((disjpat, il), id), bk, t =
      intern_cases_pattern_as_binder ~dump intern test_kind_tolerant ntnvars env
        Explicit p
    in
    ( env
    , (DAst.make ?loc:p.CAst.loc @@ GLocalPattern ((disjpat, il), id, bk, t))
      :: bl )

let intern_generalization intern env ntnvars loc bk c =
  let c = intern { env with strict_check = Some false } c in
  let fvs =
    Implicit_quantifiers.generalizable_vars_of_glob_constr ~bound:env.ids c
  in
  let env', c' =
    let abs =
      let pi =
      ...
      in
      if pi then fun { loc = loc'; v = id } acc ->
        DAst.make ?loc:(Loc.merge_opt loc' loc)
        @@ GProd
             ( Name id
             , None
             , bk
             , DAst.make ?loc:loc' @@ GHole (GBinderType (Name id))
             , acc )
      else fun { loc = loc'; v = id } acc ->
        DAst.make ?loc:(Loc.merge_opt loc' loc)
        @@ GLambda
             ( Name id
             , None
    ...
    in
    List.fold_right
      (fun ({ loc; v = id } as lid) (env, acc) ->
        let env' =
          push_name_env ~dump:true ntnvars [] env CAst.(make @@ Name id)
        in
        (env', abs lid acc))
      fvs (env, c)
  in
...
let rec expand_binders ?loc mk bl c =
  ...
  | b :: bl -> (
    ...
    | GLocalPattern ((disjpat, ids), id, bk, ty) ->
      ...
      let eqnl =
        List.map (fun pat -> CAst.make ?loc (ids, [ pat ], c)) disjpat
      in
      let c =
        DAst.make ?loc
        @@ GCases (LetPatternStyle, None, [ (tm, (Anonymous, None)) ], eqnl)
      in
...
let is_patvar_store store pat =
  ...
  | PatVar na ->
    ignore (store (CAst.make ?loc:pat.loc na));
    true
  | _ -> false

let out_patvar =
  CAst.map_with_loc (fun ?loc -> function
    | CPatAtom (Some qid) when qualid_is_ident qid -> Name (qualid_basename qid)
...
let set_type ty1 ty2 =
  ...
  | None, Some _ -> ty2
  | Some ty1, Some t2 ->
    (* An explicitly typed meta-binding binder, not supposed to be a pattern;
       checked in interp_notation *)
    user_err ?loc:t2.CAst.loc
      Pp.(
        str
          "Unexpected type constraint in notation already providing a type \
           constraint.")
...
let cook_pattern ((disjpat, ids), id) =
  ...
  let pat, na =
    match disjpat with
    | [ pat ] when is_patvar_store store pat ->
      let na = get () in
      (None, na.v)
    | _ -> (Some ((ids, disjpat), id), Name id)
  in
  (pat, na)
...
let extract_pattern_from_binder b =
  match DAst.get b with
  | GLocalDef _ ->
    user_err ?loc:b.CAst.loc (str "Local definitions not supported here.")
  | GLocalAssum (na, _, bk, t) -> (None, na, bk, t)
  | GLocalPattern (patl, id, bk, ty) ->
    let pat, na = cook_pattern (patl, id) in
    (pat, na, bk, ty)

let traverse_binder intern_pat ntnvars ((terms, _, binders, _) as subst)
    binderopt avoid (renaming, env) na ty =
  match na with
  | Anonymous -> ((renaming, env), None, Anonymous, Explicit, set_type ty None)
  | Name id -> (
    let test_kind = test_kind_tolerant in
    try
      (* We instantiate binder name with patterns which may be parsed as
         terms *)
...
type binder_action =
  ...
  | AddPreBinderIter of
      Id.t * local_binder_expr (* A binder to be internalized *)
  | AddBinderIter of Id.t * extended_glob_local_binder
    (* A binder already internalized - used for generalized binders *)
    ...
    dmap_with_loc
      (fun ?loc -> function
        ...
        | PatCstr (c, l, _) ->
          let qid =
            qualid_of_path ?loc
              (Nametab.path_of_global (GlobRef.ConstructRef c))
          in
          let hole = CAst.make ?loc @@ CHole None in
          let params =
            List.make
              (Inductiveops.inductive_nparams (Global.env ()) (fst c))
              hole
          in
          CAppExpl ((qid, None), params @ List.map term_of_pat l))
      pt
  in
  let rec extract_variables l =
    match l with
    | bnd :: l -> (
      let loc = bnd.loc in
      match DAst.get bnd with
      | GLocalAssum (Name id, _, _, _) ->
        (CAst.make ?loc @@ CRef (qualid_of_ident ?loc id, None))
        :: extract_variables l
      | GLocalDef (Name id, _, _, _) -> extract_variables l
      | GLocalDef (Anonymous, _, _, _) | GLocalAssum (Anonymous, _, _, _) ->
        user_err Pp.(str "Cannot turn \"_\" into a term.")
      | GLocalPattern (([ u ], _), _, _, _) ->
        term_of_pat u :: extract_variables l
      | GLocalPattern ((_, _), _, _, _) ->
        error_cannot_coerce_disjunctive_pattern_term ?loc ())
    | [] -> []
  in
...
let flatten_binders bl =
  let dispatch = function
    | CLocalAssum (nal, r, bk, t) ->
      List.map (fun na -> CLocalAssum ([ na ], r, bk, t)) nal
    | a -> [ a ]
  in
  List.flatten (List.map dispatch bl)

let rec adjust_env env = function
  (* We need to adjust scopes, binder blocks ... to the env expected at the
     recursive occurrence; We do an underapproximation... *)
  ...
  | NArray _
  | NList _
  | NBinderList _ -> env (* to be safe, but restart should be ok *)

let instantiate_notation_constr loc intern intern_pat
    (ntnvars : Genintern.ntnvar_status Id.Map.t) subst infos c =
  ...
  let rec aux ((terms, binderopt, iteropt) as subst') (renaming, env) c =
    ...
    | NVar id when Id.equal id ldots_var ->
      (* apply the pending sequence of letin, term iterator instances, binder
         iterator instances, and eventually terminator *)
      let rec aux_letin env = function
        | [], terminator, _ ->
          aux (terms, None, None) (renaming, env) terminator
        | AddPreBinderIter (y, binder) :: rest, terminator, iter ->
          let env, binders =
            intern_local_binder_aux ~dump:true intern ntnvars
              (adjust_env env iter, [])
              binder
          in
        ...
        | AddBinderIter (y, binder) :: rest, terminator, iter ->
          ...
          aux
            (terms, Some (y, binder), Some (rest, terminator, iter))
            (renaming, env) iter
        | AddTermIter nterms :: rest, terminator, iter ->
          (* This time, the variable [y] is the placeholder for the [binder] in
             [iter] *)
          aux (nterms, None, Some (rest, terminator, iter)) (renaming, env) iter
        | AddLetIn (na, c, t) :: rest, terminator, iter ->
          let env, (na, c, t) =
            intern_letin_binder ~dump:true intern ntnvars (adjust_env env iter)
              (na, c, t)
          in
          DAst.make ?loc
            (GLetIn (na, None, c, t, aux_letin env (rest, terminator, iter)))
        | AddNList :: rest, terminator, iter ->
          DAst.make ?loc
            (GApp
               ( DAst.make ?loc (GVar ldots_var)
               , [ aux_letin env (rest, terminator, iter) ] ))
      in
    ...
    | NList (x, y, iter, terminator, revert) ->
      ...
      let select_iter a =
        match a.CAst.v with
        | CRef (qid, None)
          when qualid_is_ident qid && Id.equal (qualid_basename qid) ldots_var
          -> AddNList
        | _ -> AddTermIter (Id.Map.add y (a, (scopt, subscopes)) terms)
      in
      ...
      let knd =
        ...
        | GBinderType (Name id as na) ->
          let na =
            ...
            with Not_found -> (
              try Name (Id.Map.find id renaming) with Not_found -> na)
          in
          GBinderType na
        | _ -> knd
      in
    ...
    | NGenarg arg ->
      ...
      let glob_of_binder ((c, _bk), (onlyident, (tmp_scope, subscopes))) =
        ...
        let test_kind =
          ...
          else test_kind_pattern_in_notation
        in
        let _, ((disjpat, _), _), _, _ty =
          intern_pat test_kind ntnvars nenv Explicit c
        in
        (* TODO: use cast? *)
        match disjpat with
      ...
      let get_glob id =
        ...
        | None -> (
          match Id.Map.find_opt id binders with
    ...
    | NBinderList (x, y, iter, terminator, revert) -> (
      try
        (* All elements of the list are in scopes (scopt,subscopes) *)
        let bl, (scopt, subscopes) = Id.Map.find x binderlists in
        (* We flatten binders so that we can interpret them at substitution
           time *)
        ...
        let l =
          List.map
            (function
              | CLocalDef (na, _, c, t) -> AddLetIn (na, c, t)
              | binder -> AddPreBinderIter (y, binder))
            bl
        in
    ...
    | t ->
      glob_constr_of_notation_constr_with_binders ?loc
        (traverse_binder intern_pat ntnvars subst binderopt avoid)
        (aux subst') ~h:binder_status_fun subinfos t
  and subst_var (terms, binderopt, _terminopt) (renaming, env) id =
    ...
    with Not_found -> (
      try
        ...
        in
        let env, ((disjpat, ids), id), bk, _ty =
          intern_pat test_kind ntnvars env bk pat
        in
        (* TODO: use cast? *)
        match disjpat with
        | [ pat ] -> glob_constr_of_cases_pattern (Global.env ()) pat
        | _ ->
          user_err Pp.(str "Cannot turn a disjunctive pattern into a term.")
      with Not_found -> (
        try
        ...
        with Not_found ->
          DAst.make ?loc
            (try GVar (Id.Map.find id renaming)
             with Not_found ->
               (* Happens for local notation joint with inductive/fixpoint
                  defs *)
               GVar id)))
  in
  aux (terms, None, None) infos c

(* Turning substitution coming from parsing and based on production into a
   substitution for interpretation and based on binding/constr distinction *)
...
let cases_pattern_of_name { loc; v = na } =
  let atom =
    match na with
    | Name id -> Some (qualid_of_ident ?loc id)
    | Anonymous -> None
  in
...
let split_by_type ids (subst : constr_notation_substitution) =
  let bind id scl l s =
    match l with
    | [] -> assert false
    | a :: l -> (l, Id.Map.add id (a, scl) s)
  in
  let (terms, termlists, binders, binderlists), subst =
    List.fold_left
      (fun ( (terms, termlists, binders, binderlists)
           , (terms', termlists', binders', binderlists') )
           (id, ((_, scl), _, typ)) ->
        match typ with
        | NtnTypeConstr ->
        ...
        | NtnTypeBinder ntn_binder_kind ->
          let onlyident, a, terms, binders =
            ...
            | NtnBinderParsedAsConstr k ->
              ...
              ( is_onlyident k
              , (cases_pattern_of_binder_as_constr a k, Explicit)
              , terms
              , binders )
        ...
        | NtnTypeBinderList ntn_binder_kind ->
          let l, termlists, binderlists =
            ...
            | NtnBinderParsedAsConstr k ->
              ...
              ( List.map
                  (fun a ->
                    CLocalPattern (cases_pattern_of_binder_as_constr a k))
                  l
              , termlists
              , binderlists )
          ...
          ( (terms, termlists, binders, binderlists)
          , (terms', termlists', binders', binderlists') ))
      (subst, (Id.Map.empty, Id.Map.empty, Id.Map.empty, Id.Map.empty))
      ids
  in
...
let split_by_type_pat ?loc ids subst =
  let bind id (_, scopes) l s =
    match l with
    | [] -> assert false
    | a :: l -> (l, Id.Map.add id (a, scopes) s)
  in
  let bind_binders id (_, scopes) l s =
    match l with
    | [] -> assert false
    | (a, Explicit) :: l -> (l, Id.Map.add id (a, scopes) s)
    | (a, (MaxImplicit | NonMaxImplicit)) :: l ->
      user_err (str "Implicit arguments not supported.")
    (* shouldn't arrive *)
  in
  let (terms, termlists, binders), subst =
    List.fold_left
      (fun ((terms, termlists, binders), (terms', termlists'))
           (id, (scl, _, typ)) ->
        match typ with
      ...
      (subst, (Id.Map.empty, Id.Map.empty))
      ids
  in
  assert (terms = [] && termlists = [] && binders = []);
  subst

let save_and_intern_notation save intern env ntnvars loc ntn fullargs =
  ...
  instantiate_notation_constr loc intern
    (intern_cases_pattern_as_binder ~dump:true intern)
    ntnvars subst (Id.Map.empty, env) c
...
let gvar (loc, id) us =
  ...
  | Some _ ->
    user_err ?loc
      (str "Variable " ++ Id.print id ++ str " cannot have a universe instance")

let intern_var env (ltacvars, ntnvars) namedctx loc id us =
  (* Is [id] a notation variable *)
  if Id.Map.mem id ntnvars then (
    if not (Id.Map.mem id env.impls) then
      set_notation_var_scope ?loc id
        (env.tmp_scope, env.scopes)
        env.ntn_binding_ids ntnvars;
    gvar (loc, id) us)
  else
    ...
    with Not_found -> (
      if
        ...
        Id.Set.mem id env.ids || Id.Set.mem id ltacvars.ltac_vars
      then gvar (loc, id) us
      else if
        Id.equal id ldots_var
        (* Is [id] the special variable for recursive notations? *)
      then
        if Id.Map.is_empty ntnvars then error_ldots_var ?loc
        else gvar (loc, id) us
      else if Id.Set.mem id ltacvars.ltac_bound then
        (* Is [id] bound to a free name in ltac (this is an ltac error
           message) *)
        user_err ?loc
        ...
        try
          ...
          Dumpglob.dump_secvar ?loc id;
          (* this raises Not_found when not a section variable *)
...
let intern_extended_global_of_qualid qid =
  let r = Nametab.locate_extended qid in
  dump_extended_global qid.CAst.loc r;
  r
    ...
    | None -> (
      ...
      with Not_found ->
        if is_id && local_univs.unb_univs then
          GLocalUniv (CAst.make ?loc:qid.loc (qualid_basename qid))
        else
...
let intern_field_ref qid =
  match
    ...
    |> Option.map (function
         | GlobRef.ConstRef c as x ->
           (x, Structure.find_from_projection (Global.env ()) c)
         | _ -> raise Not_found)
  with
  | exception Not_found ->
    Loc.raise ?loc:qid.loc (InternalizationError (NotAProjection qid))
  | None -> Loc.raise ?loc:qid.loc (InternalizationError (NotAProjection qid))
  | Some x -> x
...
let find_appl_head_data genv env (_, ntnvars) c =
  ...
  | GVar id when not (Id.Map.mem id ntnvars) -> (
    try
      let { var_impls = impls; var_scopes = argsc } =
        Id.Map.find id env.impls
      in
      ( Some (CAst.make ?loc (GlobRef.VarRef id))
      , make_implicits_list impls
      , argsc )
    with Not_found -> (None, [], []))
  | GRef (ref, _) ->
...
let check_no_explicitation l =
  ...
  | (_, Some { loc }) :: _ ->
    user_err ?loc
      (str "Unexpected explicitation of the argument of an abbreviation.")

let find_projection_data c =
  match DAst.get c with
  | GApp (r, l) -> (
    match DAst.get r with
    | GRef (GlobRef.ConstRef cst, us) ->
      Some
        ( cst
        , us
        , l
        , Structure.projection_nparams (Global.env ()) cst - List.length l )
    | _ -> None)
  | GRef (GlobRef.ConstRef cst, us) ->
    Some (cst, us, [], Structure.projection_nparams (Global.env ()) cst)
  | _ -> None
...
let intern_qualid ?(no_secvar = false) qid intern env
    (ntnvars : Genintern.ntnvar_status Id.Map.t) us args =
  let loc = qid.loc in
  ...
  | Abbrev sp ->
    ...
    let c =
      ...
      | Some _, GApp (r, arg) -> (
        ...
        | _ -> err ())
      | Some ([], [ s ]), GSort gs
        when Glob_ops.(glob_sort_eq glob_Type_sort gs) ->
      ...
      | Some ([], [ _old_level ]), GSort _new_sort ->
        ...
        user_err ?loc
          (str "Cannot change universe level of notation " ++ pr_qualid qid)
      | Some _, _ -> err ()
    in
    (c, None, args2)
...
let intern_qualid_for_pattern test_global intern_not qid pats =
  ...
  | TrueGlobal g as xref ->
    ...
    |> Option.iter (fun warn ->
           Nametab.warn_user_warn_xref ?loc:qid.loc warn (TrueGlobal g));
  ...
  | Abbrev kn as xref -> (
    let filter (vars, a) =
      match a with
      | NRef (g, _) ->
        (* Convention: do not deactivate implicit arguments and scopes for
           further arguments *)
        test_global g;
        let () = assert (List.is_empty vars) in
        Some (g, Some [], pats)
      | NApp (NRef (g, _), []) ->
        (* special case: abbreviation for @Cstr deactivates implicit
           arguments *)
        test_global g;
        let () = assert (List.is_empty vars) in
        Some (g, None, pats)
    ...
    | Some abbrev -> (
      ...
      | Some (g, pats1, pats2) ->
        Nametab.is_warned_xref xref
        |> Option.iter (fun warn ->
               Nametab.warn_user_warn_xref ?loc:qid.loc warn (Abbrev kn));
        dump_extended_global qid.loc (Abbrev kn);
        (g, true, pats1, pats2)
      | None -> raise Not_found))

let warn_nonprimitive_projection =
  CWarnings.create ~name:"my-nonprimitive-projection-syntax"
    ~category:CWarnings.CoreCategories.syntax ~default:CWarnings.Disabled
    Pp.(
      fun f ->
        pr_qualid f ++ str " used as a primitive projection but is not one.")
...
let intern_applied_reference ~isproj intern env namedctx ((_, ntnvars) as lvar)
    us args qid =
  ...
  if qualid_is_ident qid then
    ...
    with Not_found -> (
      try
        let res, realref, args2 =
          intern_qualid ~no_secvar:true qid intern env ntnvars us args
        in
      ...
      with Not_found as exn ->
        ...
        if
          Option.default true env.strict_check
          || List.exists (fun (_, e) -> Option.has_some e) args
        then
...
(** Intermediate type common to the patterns of the "in" and of the "with"
    clause of "match" *)
...
type 'a raw_cases_pattern_expr_r =
  ...
  | RCPatAtom of
      (lident
      * (Notation_term.tmp_scope_name list * Notation_term.scope_name list))
      option
...
let loc_of_multiple_pattern pl =
  Loc.merge_opt
    (cases_pattern_expr_loc (List.hd pl))
    (cases_pattern_expr_loc (List.last pl))

let check_linearity loc ids =
  match List.duplicates (fun id1 id2 -> Id.equal id1.v id2.v) ids with
  | id :: _ -> Loc.raise ?loc (InternalizationError (NonLinearPattern id.v))
  | [] -> ()
...
let check_or_pat_variables loc ids idsl =
  ...
  (* Collect remaining patterns which do not have the same variables as the
     first pattern *)
  let idsl = List.filter (fun ids' -> not (List.eq_set eq_id ids ids')) idsl in
  match idsl with
  | ids' :: _ ->
    (* Look for an [id] which is either in [ids] and not in [ids'] or in [ids']
       and not in [ids] *)
    let ids'' = List.subtract eq_id ids ids' in
    let ids'' = if ids'' = [] then List.subtract eq_id ids' ids else ids'' in
    user_err ?loc
      (strbrk
         "The components of this disjunctive pattern must bind the same \
          variables ("
      ++ Id.print (List.hd ids'').v
      ++ strbrk " is not bound in all patterns).")
  | [] -> ()

let error_wrong_numarg ?loc g ~expanded ~nargs ~expected_nassums
    ~expected_ndecls =
  let env = Global.env () in
  match g with
  | GlobRef.ConstructRef cstr ->
    error_wrong_numarg_constructor ?loc env ~cstr ~expanded ~nargs
      ~expected_nassums ~expected_ndecls
  | GlobRef.IndRef ind ->
    error_wrong_numarg_inductive ?loc env ~ind ~expanded ~nargs
      ~expected_nassums ~expected_ndecls
  | _ -> assert false
...
let check_has_letin ?loc g expanded nargs nimps tags =
  ...
  else if nargs = expected_ndecls then true
  else
    error_wrong_numarg ?loc g ~expanded ~nargs ~expected_nassums
      ~expected_ndecls

(** Do not raise NotEnoughArguments thanks to preconditions*)
let chop_params_pattern loc ind args with_letin =
  let nparams =
    if with_letin then Inductiveops.inductive_nparamdecls (Global.env ()) ind
    else Inductiveops.inductive_nparams (Global.env ()) ind
  in
  ...
  List.iter
    (fun c ->
      match DAst.get c with
      | PatVar Anonymous -> ()
      | PatVar _ | PatCstr (_, _, _) ->
        error_parameter_not_implicit ?loc:c.CAst.loc)
    params;
  args
...
let check_duplicate ?loc fields =
  ...
  | (r, _) :: _ ->
    user_err ?loc
      (str "This record defines several times the field "
      ++ pr_qualid r ++ str ".")

(** [sort_fields ~complete loc fields completer] expects a list [fields] of
    field assignments [f = e1; g = e2; ...], where [f, g] are fields of a record
    and [e1] are "values" (either terms, when interning a record construction,
    or patterns, when intering record pattern-matching). It will sort the fields
    according to the record declaration order (which is important when
    type-checking them in presence of dependencies between fields). If the
    parameter [complete] is true, we require the assignment to be complete: all
    the fields of the record must be present in the assignment. Otherwise the
    record assignment may be partial (in a pattern, we may match on some fields
    only), and we call the function [completer] to fill the missing fields; the
    returned field assignment list is always complete. *)
let sort_fields genv ~complete loc fields completer =
  ...
  | (first_field_ref, _) :: _ ->
    ...
    let build_proj idx proj =
      ...
      else (idx, proj.Structure.proj_body, proj.Structure.proj_true)
    in
    let proj_list = List.map_i build_proj 1 record.Structure.projections in
    (* now we want to have all fields assignments indexed by their place in the
       constructor *)
      ...
      | (field_ref, field_value) :: fields ->
        ...
        let remaining_projs, (field_index, _, regular) =
          let the_proj = function
            | idx, Some glob_id, _ ->
              Environ.QGlobRef.equal genv field_glob_ref
                (GlobRef.ConstRef glob_id)
            | idx, None, _ -> false
          in
          ...
          with Not_found ->
            Loc.raise ?loc
              (InternalizationError
                 (ProjectionsOfDifferentRecords (record, this_field_record)))
        in
        if (not regular) && complete then
          (* "regular" is false when the field is defined by a let-in in the
             record declaration (its value is fixed from other fields). *)
          user_err ?loc
            (str "No local fields allowed in a record construction.");
        ...
        let remaining_fields =
          let complete_field (idx, field_ref, regular) =
            if (not regular) && complete then
              ...
              None
            else Some (idx, completer idx field_ref (record.Structure.name, 1))
          in
          List.map_filter complete_field remaining_projs
        in
    ...
    let sorted_indexed_fields =
      ...
      List.sort cmp_by_index unsorted_indexed_fields
    in
...
let empty_alias = { alias_ids = []; alias_map = Id.Map.empty }

(* [merge_aliases] returns the sets of all aliases encountered at this point and
   a substitution mapping extra aliases to the first one *)
let merge_aliases aliases { loc; v = na } =
  ...
  | Name id ->
    let alias_ids =
      List.add_set
        (fun id1 id2 -> Id.equal id1.v id2.v)
        (make ?loc id) aliases.alias_ids
    in
    (* Note: assumes that add_set adds in front, see alias_of *)
    let alias_map =
      match aliases.alias_ids with
      | [] -> aliases.alias_map
      ...
      , (* Cartesian prod of the or-pats for the nth arg and the tail args *)
        List.flatten
          (List.map
             (fun (subst, p) ->
               List.map
                 (fun (subst', ptail) -> (merge_subst subst subst', p :: ptail))
                 ptaill)
             pl) ))
    idspl
    (aliases.alias_ids, [ (aliases.alias_map, []) ])

let rec subst_pat_iterator y t =
  DAst.(
    map (function
      | RCPatAtom id as p -> (
        match id with
        | Some ({ v = x }, _) when Id.equal x y -> DAst.get t
        | _ -> p)
      | RCPatCstr (id, l) -> RCPatCstr (id, List.map (subst_pat_iterator y t) l)
...
let drop_notations_pattern (test_kind_top, test_kind_inner) genv env pat =
  ...
  let ensure_kind { test_kind } ?loc g =
    try test_kind ?loc g
    with Not_found -> error_invalid_pattern_notation ?loc ()
  in
  ...
  let rec check_allowed_ref_in_pat test_kind =
    DAst.(
      with_loc_val (fun ?loc -> function
        | GVar _ | GHole _ -> ()
        ...
        | GApp (f, l) -> (
          ...
          | GRef (g, _) ->
            ...
            let nparams =
              match g with
              | IndRef ind | ConstructRef (ind, _) ->
                Inductiveops.inductive_nparams (Global.env ()) ind
              | _ -> assert false
            in
            let l =
              try List.skipn nparams l with Failure _ -> raise Not_found
            in
            List.iter (check_allowed_ref_in_pat test_kind_inner) l
          | _ -> raise Not_found)
        | _ -> raise Not_found))
  in
  (* Interpret a primitive notation (part of
     Glob_ops.cases_pattern_of_glob_constr) *)
  let rec rcp_of_glob scopes x =
    DAst.(
      map_with_loc (fun ?loc -> function
        ...
        | GApp (r, l) -> (
          ...
          | GRef (g, _) ->
            ...
            let params = make_pars ?loc g in
            (* Rem: no letins *)
            let nparams = List.length params in
            let allscs = List.skipn nparams allscs in
            let l = List.skipn nparams l in
            let pl =
              List.map2 (fun sc a -> rcp_of_glob (sc, snd scopes) a) allscs l
            in
          ...
          | _ ->
            CErrors.anomaly
              Pp.(
                str
                  "Invalid return pattern from \
                   Notation.interp_prim_token_cases_pattern_expr."))
        | _ ->
          CErrors.anomaly
            Pp.(
              str
                "Invalid return pattern from \
                 Notation.interp_prim_token_cases_pattern_expr.")))
      x
  and drop_abbrev { test_kind } ?loc scopes qid add_par_if_no_ntn_with_par
      no_impl pats =
    try
      if
        qualid_is_ident qid
        && Option.cata (Id.Set.mem (qualid_basename qid)) false env.pat_ids
        && List.is_empty pats
      then raise Not_found;
      let intern_not subst pat =
        in_not test_kind_inner qid.loc scopes subst [] pat
      in
      let g, expanded, ntnpats, pats =
        intern_qualid_for_pattern (test_kind ?loc) intern_not qid pats
      in
      match ntnpats with
      | None ->
        (* deactivate implicit *)
        let ntnpats =
          if add_par_if_no_ntn_with_par then make_pars ?loc g else []
        in
        Some (g, in_patargs ?loc scopes g expanded true ntnpats pats)
      | Some ntnpats ->
        let ntnpats =
          if add_par_if_no_ntn_with_par && ntnpats = [] then make_pars ?loc g
          else ntnpats
        in
  ...
  and in_pat ({ for_ind } as test_kind) scopes pt =
    ...
    let add_par_if_no_ntn_with_par =
      get_asymmetric_patterns () && not for_ind
    in
    let no_impl = get_asymmetric_patterns () && not for_ind in
    match pt.v with
    | CPatAlias (p, id) ->
      DAst.make ?loc @@ RCPatAlias (in_pat test_kind scopes p, id)
    | CPatRecord l -> (
      let sorted_fields =
        sort_fields genv ~complete:false loc l
          (fun _idx fieldname constructor -> CAst.make ?loc @@ CPatAtom None)
      in
    ...
    | CPatCstr (head, None, pl) -> (
      match
        drop_abbrev test_kind ?loc scopes head add_par_if_no_ntn_with_par
          no_impl pl
      with
    ...
    | CPatNotation (_, ntn, fullargs, extrargs) ->
      let ntn, (terms, termlists, binders) =
        contract_curly_brackets_pat ntn fullargs
      in
    ...
    | CPatDelimiters (depth, key, e) ->
      ...
      let scopes =
        ...
        | DelimUnboundedScope -> ([], sc :: snd scopes)
      in
    ...
    | CPatAtom (Some id) -> (
      match
        drop_abbrev test_kind ?loc scopes id add_par_if_no_ntn_with_par no_impl
          []
      with
      | Some (g, pl) -> DAst.make ?loc @@ RCPatCstr (g, pl)
      | None ->
        DAst.make ?loc
        @@ RCPatAtom (Some (make ?loc @@ find_pattern_variable id, scopes)))
    | CPatAtom None -> DAst.make ?loc @@ RCPatAtom None
    | CPatOr pl ->
      DAst.make ?loc @@ RCPatOr (List.map (in_pat test_kind scopes) pl)
    | CPatCast (_, _) ->
      (* We raise an error if the pattern contains a cast, due to current
         restrictions on casts in patterns. Cast in patterns are supported only
         in local binders and only at for_ind level. The only reason they are in
         the [cases_pattern_expr] type is that the parser needs to factor the "c
         : t" notation with user defined notations. In the long term, we will
         try to support such casts everywhere, and perhaps use them to print the
         domains of lambdas in the encoding of match in constr. This check is
         here and not in the parser because it would require duplicating the
         levels of the [pattern] rule. *)
      CErrors.user_err ?loc
        (Pp.strbrk "Casts are not supported in this pattern.")
  and in_pat_sc scopes x = in_pat test_kind_inner (x, snd scopes)
  and in_patargs ?loc scopes gr (* head of the pattern *) expanded
      (* tell if comes from a notation (for error reporting) *) no_impl
      (* tell if implicit are not expected (for asymmetric patterns, or @, or {|
         |} *)
        ntnpats
      (* prefix of patterns obtained by expansion of notations or parameter
         insertion *)
    ...
    let ntnpats_with_letin, tags =
      ...
      let tags =
        try List.skipn (List.length ntnpats_with_letin) tags
        with Failure _ ->
          error_wrong_numarg_with_notation_patterns ?loc gr (n + npats) tags
      in
      (ntnpats_with_letin, tags)
    in
    let imps =
      let imps =
        if no_impl then []
          ...
          else List.skipn_at_best n (select_stronger_impargs impls_st)
      in
      adjust_to_down tags imps None
    in
    let subscopes =
      adjust_to_down tags
        (List.skipn_at_best n (find_arguments_scope genv gr))
        []
    in
    let has_letin =
      check_has_letin ?loc gr expanded npats
        (List.count is_status_implicit imps)
        tags
    in
    let rec aux imps subscopes tags pats =
      ...
      | _, _, true :: tags, _ -> default :: aux imps subscopes tags pats
      | imp :: imps, sc :: subscopes, false :: tags, _
        when is_status_implicit imp -> default :: aux imps subscopes tags pats
  ...
  and in_not test_kind loc scopes ((terms, termlists) as fullsubst) args =
    function
    | NVar id -> (
      (* subst remembers the delimiters stack in the interpretation *)
      (* of the notations *)
      try
        ...
        in_pat test_kind
          (scopt, subscopes @ snd scopes)
          (mkAppPattern ?loc a args)
      with Not_found ->
        if Id.equal id ldots_var then
          if List.is_empty args then
            ...
            user_err
              (str
                 "Recursive notations with arguments not supported in patterns.")
        else
          anomaly
            (str "Unbound pattern notation variable: " ++ Id.print id ++ str ".")
      )
    | NApp (NVar id, ntnpl) ->
      user_err ?loc
        (str
           "Notations with an applied head variable not supported in patterns.")
    | NRef (g, _) ->
      ensure_kind test_kind ?loc g;
      DAst.make ?loc
      @@ RCPatCstr (g, in_patargs ?loc scopes g true false [] args)
    | NApp (NRef (g, _), ntnpl) ->
      ensure_kind test_kind ?loc g;
      let ntnpl =
        List.map (in_not test_kind_inner loc scopes fullsubst []) ntnpl
      in
      let no_impl =
      ...
      in
      DAst.make ?loc
      @@ RCPatCstr (g, in_patargs ?loc scopes g true no_impl ntnpl args)
    | NList (x, y, iter, terminator, revert) -> (
      if not (List.is_empty args) then
        user_err ?loc
          (strbrk
             "Application of arguments to a recursive notation not supported \
              in patterns.");
      try
        (* All elements of the list are in scopes (scopt,subscopes) *)
        let l, (scopt, subscopes) = Id.Map.find x termlists in
        let termin =
          in_not test_kind_inner loc scopes fullsubst [] terminator
        in
        List.fold_right
          (fun a t ->
            let nterms = Id.Map.add y (a, (scopt, subscopes)) terms in
            let u =
              in_not test_kind_inner loc scopes (nterms, termlists) [] iter
            in
    ...
    | NHole _ ->
      if not (List.is_empty args) then
        user_err ?loc (str "Such pattern cannot have arguments.");
      DAst.make ?loc @@ RCPatAtom None
    | NGenarg _ -> user_err ?loc (str "Quotations not supported in patterns.")
    | t -> error_invalid_pattern_notation ?loc ()
  in
  in_pat test_kind_top env.pat_scopes pat
...
let rec intern_pat genv ntnvars aliases pat =
    ...
    let pl' =
      List.map
        (fun (asubst, pl) ->
          ...
          , DAst.make ?loc
            @@ PatCstr
                 (c, chop_params_pattern loc (fst c) pl true, alias_of aliases)
          ))
        pll
    in
    (ids', pl')
  | RCPatAtom (Some ({ loc; v = id }, scopes)) ->
    ...
    set_var_is_binder ?loc id ntnvars;
    ( aliases.alias_ids
    , [ (aliases.alias_map, DAst.make ?loc @@ PatVar (alias_of aliases)) ] )
    (* TO CHECK: aura-t-on id? *)
  | RCPatAtom None ->
...
let intern_ind_pattern genv ntnvars env pat =
  ...
  let test_kind_inner ?loc = function
    ...
    | GlobRef.IndRef _ | GlobRef.ConstRef _ | GlobRef.VarRef _ ->
      (* A non-constructor global reference deep in a pattern is seen as a
         variable *)
      raise Not_found
  in
  let no_not =
  ...
  | RCPatCstr (head, pl) -> (
    ...
    | ids, [ (asubst, pl) ] ->
      (ind, ids, asubst, chop_params_pattern loc ind pl true)
    | _ -> error_bad_inductive_type ?loc ())
...
let set_hole_implicit i na imp r =
  let loc, r =
    ...
    | None -> anomaly (Pp.str "Only refs have implicits.")
  in
  DAst.make ?loc (GHole (GImplicitArg (r, (i, na), force_inference_of imp)))

let exists_implicit_name id =
  List.exists (fun imp ->
      is_status_implicit imp && Id.equal id (name_of_implicit imp))

let print_allowed_named_implicit imps =
  let l =
    List.map_filter
      (function
        | Some { impl_pos = Name id, _, _ } -> Some id
        | _ -> None)
      imps
  in
  match l with
  ...
  | l ->
    let n = List.length l in
    str " (possible "
    ++ str (String.plural n "name")
    ++ str ":" ++ spc () ++ pr_sequence Id.print l ++ str ")"

let print_allowed_nondep_implicit imps =
  let l =
    List.map_filter
      (function
        | Some { impl_pos = _, _, Some n } -> Some n
        | _ -> None)
      imps
  in
  match l with
  ...
  | l ->
    let n = List.length l in
    str " (possible "
    ++ str (String.plural n "position")
    ++ str ":" ++ spc () ++ pr_sequence Pp.int l ++ str ")"
          ...
          | ExplByName id ->
            if not (exists_implicit_name id imps) then
              user_err ?loc
                ...
                ++ print_allowed_named_implicit imps
                ++ str ".");
            if List.mem_assoc pos eargs then
              user_err ?loc
                (str "Argument name " ++ Id.print id
                ++ str " occurs more than once.")
          | ExplByPos p ->
            if not (is_nondep_implicit p imps) then
              user_err ?loc
                (str "Wrong argument position "
                ++ int p
                ++ print_allowed_nondep_implicit imps
                ++ str ".");
            if List.mem_assoc pos eargs then
              user_err ?loc
                (str "Argument at position "
                ++ int p
...
let extract_regular_arguments args =
  List.map_filter
    (function
      | a, Some pos ->
        user_err ?loc:pos.loc (str "Unexpected explicit argument.")
      | a, None -> Some a)
    args

let list_notations_and_internalize globalenv env pattern_mode
    ((_, ntnvars) as lvar) c =
  let ntnl = ref [] in
  let save_ntn locs path secpath ntn scope =
    ntnl := (locs, path, secpath, ntn, scope) :: !ntnl
  in
  let rec intern env =
    CAst.with_loc_val (fun ?loc -> function
      | CRef (ref, us) ->
        let c, _ =
          intern_applied_reference ~isproj:false intern env
            (Environ.named_context_val globalenv)
            lvar us [] ref
        in
      ...
      | CFix ({ CAst.loc = locid; v = iddef }, dl) ->
        ...
        let n =
          ...
          with Not_found as exn ->
            ...
            Exninfo.iraise
              (InternalizationError (UnboundFixName (false, iddef)), info)
        in
                    ...
                    | { CAst.v; loc } -> (
                      ...
                      | _ ->
                        user_err ?loc
                          Pp.(
                            str
                              "Well-founded induction requires Program \
                               Fixpoint or Function.")))
                  recarg
              in
                  ...
                  (fun recarg ->
                    ...
                    try
                        ...
                        (fun n (id, _, _, body, _) ->
                          ...
                          | Some _, true ->
                            CErrors.user_err ?loc
                              (Name.print id
                              ++ str
                                   " must be a proper parameter and not a \
                                    local definition.")
                          | None, false -> n + 1
                      ...
                      CErrors.user_err ?loc:recarg.loc
                        (str "No parameter named " ++ Id.print recarg.v
                       ++ str ".")
                    with Found k -> k)
                  recarg
              in
              let bl_impls = remember_binders_impargs env' bl in
              (n, bl, intern_type env' ty, bl_impls))
            dl
        in
        ...
        let env_rec =
          List.fold_left_i
            (fun i en name ->
              ...
              push_name_env ~dump:true ntnvars impls en (CAst.make @@ Name name))
            0 env lf
        in
        let idl =
          Array.map2
            (fun (_, _, _, _, _, bd) (n, bl, ty, before_impls) ->
              (* We add the binders common to body and type to the
                 environment *)
              let env_body = restore_binders_impargs env_rec before_impls in
              (n, bl, ty, intern { env_body with tmp_scope = [] } bd))
            dl idl_temp
        in
        DAst.make ?loc
        @@ GRec
             ( GFix (Array.map (fun (ro, _, _, _) -> ro) idl, n)
             , Array.of_list lf
      ...
      | CCoFix ({ CAst.loc = locid; v = iddef }, dl) ->
        ...
        let n =
          ...
          with Not_found as exn ->
            ...
            let info = Option.cata (Loc.add_loc info) info locid in
            Exninfo.iraise
              (InternalizationError (UnboundFixName (true, iddef)), info)
        in
        let env = restart_lambda_binders env in
        let idl_tmp =
          Array.map
            (fun ({ CAst.loc; v = id }, _, bl, ty, _) ->
              ...
              let bl_impls = remember_binders_impargs env' bl in
              (bl, intern_type env' ty, bl_impls))
            dl
        in
        let env_rec =
          List.fold_left_i
            (fun i en name ->
              ...
              push_name_env ~dump:true ntnvars
                (impls_type_list ~args:cofix_args binder_index tyi)
                en (CAst.make @@ Name name))
            0 env lf
        in
        let idl =
          Array.map2
            (fun (_, _, _, _, bd) (b, c, bl_impls) ->
              (* We add the binders common to body and type to the
                 environment *)
              let env_body = restore_binders_impargs env_rec bl_impls in
              (b, c, intern { env_body with tmp_scope = [] } bd))
            dl idl_tmp
        in
        DAst.make ?loc
        @@ GRec
      ...
      | CProdN (bl, c2) ->
        let env', bl =
          List.fold_left intern_local_binder (switch_prod_binders env, []) bl
        in
      ...
      | CLambdaN (bl, c2) ->
        let env', bl =
          List.fold_left intern_local_binder
            (reset_tmp_scope (switch_lambda_binders env), [])
            bl
        in
      ...
      | CLetIn (na, c1, t, c2) ->
        ...
        @@ GLetIn
             ...
             , inc1
             , int
             , intern_restart_binders
                 (push_name_env ~dump:true ntnvars (impls_term_list 1 inc1) env
                    na)
                 c2 )
      | CNotation (_, (InConstrEntry, "- _"), ([ a ], [], [], []))
        when is_non_zero a ->
        let p =
          match a.CAst.v with
          | CPrim (Number (_, p)) -> p
      ...
      | CNotation (_, ntn, args) ->
        let c =
          save_and_intern_notation save_ntn intern env ntnvars loc ntn args
        in
        apply_impargs env loc c []
      | CGeneralization (b, c) ->
      ...
      | CDelimiters (depth, key, e) ->
        ...
        let env =
          ...
          | DelimUnboundedScope ->
            { env with tmp_scope = []; scopes = sc :: env.scopes }
        in
        intern env e
      | CProj (expl, f, args, c) -> intern_proj ?loc env expl f args c []
      ...
      | CApp (f, args) -> (
        ...
        | CNotation (_, ntn, ntnargs) ->
          let c =
            save_and_intern_notation save_ntn intern env ntnvars loc ntn ntnargs
          in
          ...
          sort_fields globalenv ~complete:true loc fs
            (fun _idx fieldname constructorname ->
              ...
              let fieldinfo : Evar_kinds.record_field =
                { fieldname = Option.get fieldname
                ; recordname = inductive_of_constructor constructorname
                }
              in
              CAst.make ?loc
              @@ CHole
      ...
      | CCases (sty, rtnpo, tms, eqns) ->
        let as_in_vars =
          List.fold_left
            (fun acc (_, na, inb) ->
              Option.fold_left
                (fun acc { CAst.v = y } -> Name.fold_right Id.Set.add y acc)
                acc na)
            Id.Set.empty tms
        in
        (* as, in & return vars *)
        let forbidden_vars =
          Option.cata free_vars_of_constr_expr as_in_vars rtnpo
        in
        let tms, ex_ids, aliases, match_from_in =
          List.fold_right
            (fun citm (inds, ex_ids, asubst, matchs) ->
              let (tm, ind), extra_id, (ind_ids, alias_subst, match_td) =
                intern_case_item env forbidden_vars citm
              in
              ( (tm, ind) :: inds
              , Id.Set.union ind_ids
                  (Option.fold_right Id.Set.add extra_id ex_ids)
        ...
        let env' =
          Id.Set.fold
            (fun var bli ->
              push_name_env ~dump:true ntnvars [] bli (CAst.make @@ Name var))
        ...
        let rtnpo =
          match stripped_match_from_in with
          | [] ->
            Option.map
              (intern_type (slide_binders env'))
              rtnpo (* Only PatVar in "in" clauses *)
          | l ->
            (* Build a return predicate by expansion of the patterns of the "in"
               clause *)
            let thevars, thepats = List.split l in
            let sub_rtn = (* Some (GSort (Loc.ghost,GType None)) *) None in
            let sub_tms =
              List.map
                (fun id -> (DAst.make @@ GVar id, (Name id, None)))
                thevars (* "match v1,..,vn" *)
            in
            let main_sub_eqn =
              CAst.make
              @@ ( []
                 ...
                 , (* "|p1,..,pn" *)
                   Option.cata
                     (intern_type_no_implicit env')
                     (DAst.make ?loc @@ GHole GCasesType)
                     rtnpo )
              (* "=> P" if there were a return predicate P, and "=> _"
                 otherwise *)
            in
            let catch_all_sub_eqn =
              if List.for_all (irrefutable globalenv) thepats then []
              else
                [ CAst.make
                  @@ ( []
                     , List.make (List.length thepats)
                         (DAst.make @@ PatVar Anonymous)
                     , (* "|_,..,_" *)
                       DAst.make @@ GHole GImpossibleCase )
                ]
              (* "=> _" *)
            in
            Some
        ...
        DAst.make ?loc @@ GCases (sty, rtnpo, tms, List.flatten eqns')
      | CLetTuple (nal, (na, po), b, c) ->
        let env' = reset_tmp_scope env in
        (* "in" is None so no match to add *)
        let (b', (na', _)), _, _ =
          intern_case_item env' Id.Set.empty (b, na, None)
        in
        let p' =
          Option.map
            (fun u ->
              let env'' =
                push_name_env ~dump:true ntnvars [] env' (CAst.make na')
              in
              intern_type (slide_binders env'') u)
            po
        in
        DAst.make ?loc
        @@ GLetTuple
             ( List.map (fun { CAst.v } -> v) nal
             , (na', p')
             , b'
             , intern
                 (List.fold_left (push_name_env ~dump:true ntnvars []) env nal)
                 c )
      | CIf (c, (na, po), b1, b2) ->
        let env' = reset_tmp_scope env in
        let (c', (na', _)), _, _ =
          intern_case_item env' Id.Set.empty (c, na, None)
        in
        (* no "in" no match to ad too *)
        let p' =
          Option.map
            (fun p ->
              let env'' =
                push_name_env ~dump:true ntnvars [] env (CAst.make na')
              in
              intern_type (slide_binders env'') p)
            po
        in
        DAst.make ?loc @@ GIf (c', (na', p'), intern env b1, intern env b2)
      | CHole k ->
        let k =
          match k with
          | None ->
      ...
      | CGenarg gen ->
        ...
        Id.Map.iter
          (fun x status -> status.Genintern.ntnvar_used_as_binder <- false)
          ntnvars;
      ...
      | CPatVar n when pattern_mode ->
        DAst.make ?loc @@ GPatVar (Evar_kinds.SecondOrderPatVar n)
      | CEvar (n, []) when pattern_mode ->
        DAst.make ?loc @@ GPatVar (Evar_kinds.FirstOrderPatVar n.CAst.v)
      (* end *)
      (* Parsing existential variables *)
      | CEvar (n, l) ->
        DAst.make ?loc @@ GEvar (n, List.map (on_snd (intern_no_implicit env)) l)
      | CPatVar _ -> Loc.raise ?loc (InternalizationError IllegalMetavariable)
      (* end *)
      | CSort s ->
        DAst.make ?loc @@ GSort (intern_sort ~local_univs:env.local_univs s)
      | CCast (c1, k, c2) ->
        ...
        DAst.make ?loc @@ GCast (c1, k, c2)
      | CArray (u, t, def, ty) ->
        DAst.make ?loc
        @@ GArray
             ( intern_instance ~local_univs:env.local_univs u
             , Array.map (intern env) t
             , intern env def
             , intern env ty ))
  and intern_type env = intern (set_type_scope env)
  and intern_type_no_implicit env =
    intern (restart_no_binders (set_type_scope env))
  and intern_no_implicit env = intern (restart_no_binders env)
  and intern_restart_binders env = intern (restart_lambda_binders env)
  and intern_type_restart_binders env =
    intern (restart_prod_binders (set_type_scope env))
  and intern_local_binder env bind :
      intern_env * Glob_term.extended_glob_local_binder list =
  ...
  and intern_multiple_pattern env n pl =
    ...
    let idsl_pll =
      List.map
        (intern_cases_pattern test_kind_tolerant globalenv ntnvars env
           empty_alias)
        pl
    in
  ...
  and intern_case_item env forbidden_names_for_gen (tm, na, t) =
    ...
    let extra_id, na =
      ...
      | GVar id, None when not (Id.Map.mem id (snd lvar)) ->
        (Some id, CAst.make ?loc @@ Name id)
      | GRef (GlobRef.VarRef id, _), None -> (Some id, CAst.make ?loc @@ Name id)
      | _, None -> (None, CAst.make Anonymous)
      | _, Some ({ CAst.loc; v = na } as lna) -> (None, lna)
    in
    (* the "in" part *)
    let match_td, typ =
      match t with
      | Some t ->
        let ind, ind_ids, alias_subst, l =
          intern_ind_pattern globalenv ntnvars
            (env_for_pattern (set_type_scope env))
            t
        in
        ...
        (* for "in Vect n", we answer (["n","n"],[(loc,"n")])
           ...
           for "in Vect (S n)", we answer ((match over "m", relevant branch is
           "S n"), abstract over "m") = ([("m","S n")],[(loc,"m")]) where "m" is
           generated from the canonical name of the inductive and outside of
           {forbidden_names_for_gen} *)
        let match_to_do, nal =
          let rec canonize_args case_rel_ctxt arg_pats forbidden_names match_acc
              var_acc =
            let add_name l = function
              | { CAst.v = Anonymous } -> l
              | { CAst.loc; v = Name y as x } ->
                (y, DAst.make ?loc @@ PatVar x) :: l
            in
            match (case_rel_ctxt, arg_pats) with
            | [], [] -> (add_name match_acc na, var_acc)
            | ( (LocalAssum (cano_name, ty) | LocalDef (cano_name, _, ty)) :: t
              , c :: tt ) -> (
              match DAst.get c with
              | PatVar x ->
                let loc = c.CAst.loc in
                canonize_args t tt forbidden_names
                  (add_name match_acc CAst.(make ?loc x))
                  (CAst.make ?loc x :: var_acc)
              | _ ->
                let fresh =
                  Namegen.next_name_away_with_default_using_types globalenv
                    (Evd.from_env globalenv) "iV" cano_name.binder_name
                    forbidden_names (EConstr.of_constr ty)
                in
                canonize_args t tt
                  (Id.Set.add fresh forbidden_names)
                  ((fresh, c) :: match_acc)
                  ((CAst.make ?loc:(cases_pattern_loc c) @@ Name fresh)
                  :: var_acc))
            | _ -> assert false
          in
          let _, args_rel =
            List.chop nparams (List.rev mip.Declarations.mind_arity_ctxt)
          in
          canonize_args args_rel l forbidden_names_for_gen [] []
        in
        ( ( Id.Set.of_list (List.map (fun id -> id.CAst.v) ind_ids)
          , alias_subst
          , match_to_do )
        , Some
            (CAst.make ?loc:(cases_pattern_expr_loc t)
               (ind, List.rev_map (fun x -> x.v) nal)) )
      | None -> ((Id.Set.empty, Id.Map.empty, []), None)
    in
  ...
  and intern_proj ?loc env expl (qid, us) args1 c args2 =
    ...
    | Some (p, us, args0, nexpectedparams) ->
      ...
      let imps1, imps2 =
        if expl then ([], [])
          ...
          let ngivenparams =
            List.count (fun (_, x) -> Option.is_empty x) args1
          in
          let nextraargs = List.length args2 in
          match
            select_impargs_size_for_proj ~nexpectedparams ~ngivenparams
              ~nextraargs impls
          with
          ...
          | Inr l ->
            ...
            let n =
              ...
              | _ -> 2
            in
            (* singular only when l = [1] *)
            user_err ?loc:qid.CAst.loc
              (str "Projection " ++ pr_qualid qid ++ str " expected "
             ++ pr_choice int l
              ++ str (String.plural n " explicit parameter")
              ++ str ".")
      in
      let subscopes1, subscopes2 = List.chop (nexpectedparams + 1) subscopes in
      let c, args1 =
        List.sep_last
          (intern_impargs head env imps1 subscopes1 (args1 @ [ (c, None) ]))
      in
    ...
    | None ->
      (* Tolerate a use of t.(f) notation for an ordinary application until a
         decision is taken about it *)
      if expl then
        intern env
          (CAst.make ?loc
             (CAppExpl
                ((qid, us), List.map fst args1 @ (c :: List.map fst args2))))
      else
        intern env
          (CAst.make ?loc
             (CApp
                ( CAst.make ?loc:qid.CAst.loc (CRef (qid, us))
                , args1 @ ((c, None) :: args2) )))
    ...
    if !parsing_explicit then
      if List.is_empty eargs then intern_args env subscopes rargs
        ...
        user_err
          Pp.(
            str
              "Arguments given by name or position not supported in explicit \
               mode.")
    else
      let rec aux n imps subscopes eargs rargs =
        ...
        | imp :: imps', rargs when is_status_implicit imp -> (
          try
            let eargs', (_, (_, a)) =
              List.extract_first (fun (pos, a) -> match_implicit imp pos) eargs
            in
            intern_no_implicit enva a
            :: aux (n + 1) imps' subscopes' eargs' rargs
          with Not_found ->
            if
              List.is_empty rargs && List.is_empty eargs
              && not (maximal_insertion_of imp)
            then
              (* Less regular arguments than expected: complete *)
              (* with implicit arguments if maximal insertion is set *)
              []
            else
              set_hole_implicit n (get_implicit_name n allimps) imp head
              :: aux (n + 1) imps' subscopes' eargs rargs)
        | imp :: impl', a :: rargs' ->
        ...
        | imp :: impl', [] ->
          (if not (List.is_empty eargs) then
             let pr_position = function
               ...
               | ExplByPos n -> str "position " ++ int n
             in
             let pos, (loc, _) = List.hd eargs in
             user_err ?loc
               (str
                  "Not enough non implicit arguments to accept the argument \
                   bound to "
               ++ pr_position pos ++ str "."));
          []
        | [], rargs ->
  ...
  and smart_gapp f loc = function
    ...
    | l -> (
      ...
      match DAst.get f with
      | GApp (g, args) ->
        DAst.make ?loc:(Loc.merge_opt loc' loc) @@ GApp (g, args @ l)
      | _ ->
        DAst.make ?loc:(Loc.merge_opt (loc_of_glob_constr f) loc) @@ GApp (f, l)
      )
  ...
  NewProfile.profile "intern"
    (fun () ->
      let c = intern env c in
      (c, !ntnl))
    ()
...
let my_intern_constr env lvar acc c =
  fst (list_notations_and_internalize env acc false lvar c)

let default_internalization_env ids bound_univs impl_env =
  { ids
  ; strict_check = Some true
...
let push_auto_implicit env sigma t int_env id =
  ...
  let imps =
    List.map
      (fun imp ->
        CAst.make (Option.map (fun imp -> (pi1 imp.impl_pos, imp.impl_max)) imp))
      var_info.var_impls
  in
  let imps =
    compute_internalization_data env sigma ~silent:false id
      var_info.var_intern_typ t imps
  in
  (* add automatic implicit arguments to manual ones *)
  { int_env with impls = Id.Map.add id imps int_env.impls }

let interp_context_evars_gen ?(program_mode = false)
    ?(unconstrained_sorts = false) ?(impl_env = empty_internalization_env)
    ?(share = false) ?(autoimp_enable = true) ~dump env sigma make_decl
    push_decl bl =
  let lvar = (empty_ltac_sign, Id.Map.empty) in
  let ids =
    (* We assume all ids around are parts of the prefix of the current context
       being interpreted *)
    extract_ids env
  in
  let int_env =
    default_internalization_env ids (Evd.universe_binders sigma) impl_env
  in
  let flags =
    { Pretyping.all_no_fail_flags with program_mode; unconstrained_sorts }
  in
  let int_env, (env, sigma, bl, impls) =
    List.fold_left
      (fun (int_env, acc) b ->
        let int_env, bl =
          intern_local_binder_aux ~dump
            (my_intern_constr env lvar)
            Id.Map.empty (int_env, []) b
        in
        let int_env, _, acc =
          List.fold_right
            (fun b' (int_env, first, (env, sigma, params, impls)) ->
              let na, _, bk, b', t = glob_local_binder_of_extended b' in
              let sigma, t =
                match (first, b) with
                | Some t, CLocalAssum (_, _, Default _, _) when share ->
                  (sigma, t)
                | _ ->
                  let t' =
                    if Option.is_empty b' then
                      locate_if_hole ?loc:(loc_of_glob_constr t) na t
                    else t
                  in
                  (* useful? *)
                  let sigma, t, _ =
                    Pretyping.ise_pretype_gen flags env sigma empty_lvar IsType
                      t'
                  in
                  (sigma, t)
              in
              ...
              | None ->
                let int_env =
                  if autoimp_enable then
                    Name.fold_left (push_auto_implicit env sigma t) int_env na
                  else int_env
                in
              ...
              | Some b ->
                assert (bk = Explicit);
                let sigma, c, _ =
                  Pretyping.ise_pretype_gen flags env sigma empty_lvar
                    (OfType t) b
                in
...
let interp_context_evars ?program_mode ?unconstrained_sorts ?impl_env ?share env
    sigma bl =
  interp_context_evars_gen ?program_mode ?unconstrained_sorts ?impl_env ?share
    ~autoimp_enable:false ~dump:true env sigma
    (fun d -> d)
    EConstr.push_rel bl

let list_notations_in_statement_lemma ~program_mode env0 evd thm =
  let _, (bl, t) = thm in
  let evd, (impls, ((env, _), _)) =
    interp_context_evars ~program_mode env0 evd bl
  in
  let intern_env =
    { ids = extract_ids env
    ; strict_check = None
    ; local_univs = { bound = Evd.universe_binders evd; unb_univs = true }
    ; tmp_scope = Notation.current_type_scope_names ()
    ; scopes = []
    ; impls
    ; binder_block_names = Some (Some AbsPi)
    ; ntn_binding_ids = Id.Set.empty
  ...
  in
  snd
    (list_notations_and_internalize env intern_env false
       (empty_ltac_sign, Id.Map.empty)
       t)
